...
saving_path ='../../results/classify/2_0_class_into_gpcr_not_classA'
...
gpcr_classes = ['B1_secretin', 'B2_Adhesion','C_Glutamate','F_Frizzled','T_Taste']
for gpcr_class in gpcr_classes:
    file_error.write('{}\n'.format(gpcr_class))
    #list up the gpcrs that belong to class A
    file_class = open('../../data/GPCRdb/GPCRdb_alignment_{}.csv'.format(gpcr_class),'r')
    ...
    for common_name in A_NAME:
        ...
        for dataset in datasets:
            if not '{}/{}'.format(saving_path,dataset):
    ...
    for dataset in datasets:
        ...
        file_variants = open('../../results/filter/4_0_filter_vcf_on_other_classes_canonical_isoform/{}/on_{}_canonical_isoforms.vcf'.format(dataset,gpcr_class),'r')
