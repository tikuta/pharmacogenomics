...
saving_path = '../../../results/graph/Fig_3/b_variant_AC_accumulation'
    ...
    for variant in variants:
        ...
        for i in info:
            if i.startswith('AC='):
                ...
                COUNT[generic] = COUNT[generic] + ac
...
ax.set_ylabel('Allele Count', fontname='Arial')

fig.savefig('{}/a_variant_accumulation.svg'.format(saving_path))

print(counts_sort)

file_data = open('{}/data.csv'.format(saving_path),'w')
for j in range(len(generic_sort)):
    file_data.write('{}\t{}\n'.format(generic_sort[j], counts_sort[j]))