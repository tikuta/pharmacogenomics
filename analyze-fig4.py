...
def analyze_jpn_vs_global():
    ...
    likely_pathogenic_freqs = [(jpn_vars[k].snv.AF if k in jpn_vars.keys() else 0, global_vars[k].snv.AF if k in global_vars.keys() else 0) for k in likely_pathogenic_keys]
    ambigous_freqs = [(jpn_vars[k].snv.AF if k in jpn_vars.keys() else 0, global_vars[k].snv.AF if k in global_vars.keys() else 0) for k in ambigous_keys]
    likely_benign_freqs = [(jpn_vars[k].snv.AF if k in jpn_vars.keys() else 0, global_vars[k].snv.AF if k in global_vars.keys() else 0) for k in likely_benign_keys]
    ...
    # Plot only AF > 0.25
    ax.scatter(*np.array([freqs for freqs in likely_benign_freqs if freqs[0] > 0.25 or freqs[1] > 0.25]).T, marker='.', color='tab:gray', label="likely benign", zorder=1)
    ax.scatter(*np.array([freqs for freqs in ambigous_freqs if freqs[0] > 0.25 or freqs[1] > 0.25]).T, marker='.', color='tab:orange', label="ambigous", zorder=10)
    ax.scatter(*np.array([freqs for freqs in likely_pathogenic_freqs if freqs[0] > 0.25 or freqs[1] > 0.25]).T, marker='D', s=10, color='tab:orange', label="likely pathogenic", zorder=100)
